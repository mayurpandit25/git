...
resource "aws_instnace" "my-ec2" {
    ami = var.ami 
    instance_type = t2.micro
    key_pair = id_rsa 
    vpc_security_group_id = ["place your vpc here"]
}
...
resource "aws_default_route_table" "my-rt" {
    ...
    tags = {
        name = "${var.project}-myroute"
    } 
}

resource "aws_security_group" "my-sg" {
    name = "${var.project}-sg"
    description = all httpd port allow 
    vpc_id = aws_vpc.my-vpc.id 
...
resource "aws_instance" "pub-instace" {
    ami = var.ami 
    instance_type = var.instance_type 
    key_name = var.key_pair
    subnet_id = aws_subnet.pub-subnet.id 
    vpc_security_group_ids = [aws_security_grop.my-sg.id]
    ...
    depends_on = [ aws_secuiry_grop.my-sg ]
}
...
output "public_ip" {
    value = aws_isntance.pub-instance.id 
}
